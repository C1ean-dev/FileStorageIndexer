...
Handles the batch scan operation with progress bar.
"""
...
class ScanBatchHandler(BaseHandler):
    """
    Handler for batch scan operations.

    Provides scanning with a determined progress bar.
    """

    def get_description(self) -> str:
        """Get description of this handler."""
        return "[BATCH] Escaneamento com barra de progresso determinada"

    def handle(self, *args, **kwargs) -> None:
        """Handle the batch scan operation."""
        self._display_header("[BATCH] ESCANEAMENTO")
        print("Modo com barra de progresso determinada.")
        ...
        try:
            ...
            request = ScanRequest(
                path=path,
                scan_mode=ScanMode.BATCH,
                update_existing=False
            )

            # Execute scan
            use_case = self.container.get_scan_files_batch_use_case()
            ...
            print(f"[ERRO] Erro durante escaneamento batch: {str(e)}")
    ...
    def _display_scan_results(self, result: dict) -> None:
        ...
        print(f"\n{'='*50}")
        print("RESULTADOS DO ESCANEAMENTO BATCH")
        print(f"{'='*50}")
